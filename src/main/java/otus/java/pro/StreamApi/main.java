...
import otus.java.pro.StreamApi.TaskFactory.Task;
...
public class main {
    public static void main(String[] args) {
        ...
        List<Task> inProgressTasks = taskWorker.getTasksByStatus(Status.IN_PROGRESS);
        System.out.printf("Tasks in status \"%s\": %s \n", Status.IN_PROGRESS, inProgressTasks);
        List<Task> openedTasks = taskWorker.getTasksByStatus(Status.OPEN);
        System.out.printf("Tasks in status \"%s\": %s \n", Status.OPEN, openedTasks);
        List<Task> closedTasks = taskWorker.getTasksByStatus(Status.CLOSED);
